...
window.angular && (function(angular) {
  ...
  angular.module('app.kvm').controller('kvmController', [
    '$scope', '$location', '$window', '$log',
    function($scope, $location, $window, $log) {
      ...
      try {
        rfb = new RFB(target, 'wss://' + host + ':' + port + '/kvmws', {});
      ...
      function status(text, level) {
        var status_bar =
            angular.element(document.querySelector('#noVNC_status_bar'))[0];
        // Need to check if the status bar still exists.  On page change, it
        // gets destroyed when we swap to a different view.  The system will
        // disconnect async
        if (status_bar) {
        ...
        var status =
            angular.element(document.querySelector('#noVNC_status'))[0];
        switch (level) {
          case 'normal':
          case 'warn':
          case 'error':
