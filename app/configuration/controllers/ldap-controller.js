  ...
  angular.module('app.configuration').controller('ldapController', [
    ...
    function($scope, APIUtils, $q, toastService) {
      ...
      $scope.loadLdap = function() {
        ...
        var getLdapProperties = APIUtils.getAllUserAccountProperties().then(
            function(data) {
              $scope.ldapProperties = {
                'ServiceEnabled':
                   data.LDAP.ServiceEnabled ? data.LDAP.ServiceEnabled : data.ActiveDirectory.ServiceEnabled ? data.ActiveDirectory.ServiceEnabled : false,
                'LDAPServiceEnabled': data.LDAP.ServiceEnabled,
                'ADServiceEnabled': data.ActiveDirectory.ServiceEnabled,
                'EnabledServiceType':
                data.LDAP.ServiceEnabled ? 'ldap': data.ActiveDirectory.ServiceEnabled ? 'ad': "",
                'ServiceAddresses':
                   data.LDAP.ServiceEnabled ? data.LDAP.ServiceAddresses : data.ActiveDirectory.ServiceEnabled ? data.ActiveDirectory.ServiceAddresses : [],
                'useSSL':
                   $scope.isSSL(data.LDAP.ServiceEnabled ? data.LDAP.ServiceAddresses[0] : data.ActiveDirectory.ServiceAddresses[0]),
                'Username':
                    data.LDAP.ServiceEnabled ? data.LDAP.Authentication.Username : data.ActiveDirectory.ServiceEnabled ? data.ActiveDirectory.Authentication.Username : "",
                'BaseDistinguishedNames':
                    data.LDAP.ServiceEnabled ? data.LDAP.LDAPService.SearchSettings.BaseDistinguishedNames : data.ActiveDirectory.ServiceEnabled ?   data.ActiveDirectory.LDAPService.SearchSettings.BaseDistinguishedNames : [],
                'GroupsAttribute':
                    data.LDAP.ServiceEnabled ? data.LDAP.LDAPService.SearchSettings.GroupsAttribute : data.ActiveDirectory.ServiceEnabled ? data.ActiveDirectory.LDAPService.SearchSettings.GroupsAttribute : "",
                'UsernameAttribute':
                    data.LDAP.ServiceEnabled ? data.LDAP.LDAPService.SearchSettings.UsernameAttribute : data.ActiveDirectory.ServiceEnabled ? data.ActiveDirectory.LDAPService.SearchSettings.UsernameAttribute : "",
                'AuthenticationType':
                    data.LDAP.ServiceEnabled ?  data.LDAP.Authentication.AuthenticationType : data.ActiveDirectory.Authentication.AuthenticationType,
              };
      ...
      $scope.saveLdapSettings = function() {
        ...
        LDAPService.SearchSettings.BaseDistinguishedNames = $scope.ldapProperties.BaseDistinguishedNames;
        LDAPService.SearchSettings.GroupsAttribute = $scope.ldapProperties.GroupsAttribute;
        LDAPService.SearchSettings.UsernameAttribute = $scope.ldapProperties.UsernameAttribute;
