    ...
    angular.module('app.configuration').controller('virtualMediaController', [
      ...
      function($scope, APIUtils, toastService, dataService, nbdServerService) {
        ...
        $scope.startVM = function(index) {
          ...
          const file = $scope.devices[index].file;
          const id = $scope.devices[index].id;
          const host = dataService.getHost().replace('https://', '');
        ...
        $scope.stopVM = function(index) {
          ...
          const server = $scope.devices[index].nbdServer;
          server.stop();
        };
        ...
        function findExistingConnection(vmDevice) {
          ...
          const existingConnectionsMap = nbdServerService.getExistingConnections();
...
function NBDServer(endpoint, file, id) {
  ...
  this._on_ws_message = function(ev) {
    ...
    for (;;) {
      const handler = this.recv_handlers[this.state];
      if (!handler) {
      ...
      const consumed = handler(this.msgbuf);
      if (consumed < 0) {
        console.log(
  ...
  this._negotiate = function() {
    ...
    const data = new DataView(buf, 0, 18);
  ...
  this._handle_cflags = function(buf) {
    ...
    const data = new DataView(buf, 0, 4);
  ...
  this._create_cmd_response = function(req, rc, data = null) {
    ...
    const resp = new ArrayBuffer(len);
    const view = new DataView(resp, 0, 16);
  ...
  this._handle_cmd = function(buf) {
    ...
    if (err) {
      ...
      const resp = this._create_cmd_response(req, err);
      this.ws.send(resp);
    }
  ...
  this._handle_cmd_read = function(req) {
    const offset = req.offset_msB * 2 ** 32 + req.offset_lsB;
    ...
    const blob = this.file.slice(offset, offset + req.length);
    const reader = new FileReader();

    reader.onload = function(ev) {
      const reader = ev.target;
      if (reader.readyState != FileReader.DONE) return;
      const resp = this._create_cmd_response(req, 0, reader.result);
      this.ws.send(resp);
    }.bind(this);
    ...
    reader.onerror = function(ev) {
      ...
      const resp = this._create_cmd_response(req, EIO);
      this.ws.send(resp);
    }.bind(this);
