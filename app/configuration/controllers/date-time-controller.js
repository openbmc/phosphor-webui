  ...
  (function(angular) {
    ...
    angular.module('app.configuration').controller('dateTimeController', [
      ...
      function($scope, $window, APIUtils, $route, $q, toastService) {
        ...
        const getTimePromise = APIUtils.getTime().then(
          function(data) {
            ...
            if (
              ...
              $scope.bmc.timezone =
                getUserTimezone($scope.bmc.date) +
                ' ' +
                createOffset($scope.bmc.date);
            }
            if (
              data.data[timePath + 'host'] &&
              data.data[timePath + 'host'].hasOwnProperty('Elapsed')
            ) {
              $scope.host.date = new Date(
                data.data[timePath + 'host'].Elapsed / 1000
              );
            ...
            if (
              data.data[timePath + 'owner'] &&
              data.data[timePath + 'owner'].hasOwnProperty('TimeOwner')
            ) {
              $scope.time.owner = data.data[timePath + 'owner'].TimeOwner.split(
                '.'
              ).pop();
            }
            if (
              data.data[timePath + 'sync_method'] &&
              data.data[timePath + 'sync_method'].hasOwnProperty(
                'TimeSyncMethod'
              )
            ) {
              $scope.time.mode = data.data[
                timePath + 'sync_method'
              ].TimeSyncMethod.split('.').pop();
            }
          },
          function(error) {
        ...
        const promises = [getTimePromise, getNTPPromise];
