    ...
    angular.module('app.configuration').controller('firmwareController', [
      ...
      function(
        ...
        $scope.upload = function() {
          if ($scope.file) {
            $scope.uploading = true;
            APIUtils.uploadImage($scope.file).then(
              function(response) {
                $scope.uploading = false;
                toastService.success(
                  'Image file "' + $scope.file.name + '" has been uploaded'
                );
                $scope.file = '';
                $scope.loadFirmwares();
              },
