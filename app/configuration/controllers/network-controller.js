      ...
      $scope.setNetworkSettings = function() {
        ...
        if (!$scope.interface.DHCPEnabled) {
          for (var i in $scope.interface.ipv4.values) {
            ...
            // This netmask prefix length match only work with IPV4
        ...
        if (promises.length) {
          $q.all(promises).finally(function() {
            ...
            // edit is a delete then an add and we can't calculate the interface
            // id (e.g. 5c083707) beforehand and it is not returned,
            // openbmc#3227, reload the page which makes a REST call. Do this
            // for all network  changes due to the possibility of a set network
            // failing even though it returned okay, openbmc#1641,
            // and to update dataService and old_interface to know which
            // data has changed if the user continues to edit network settings.
            $timeout(function() {
              $route.reload();
            }, 4000);
          });
