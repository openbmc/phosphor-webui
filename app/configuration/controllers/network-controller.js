    ...
    angular.module('app.configuration').controller('networkController', [
      ...
      function(
        ...
        $scope.setNetworkSettings = function() {
          ...
          if (
            $scope.interface.MACAddress.toLowerCase() !=
          ...
          $scope.interface.Nameservers = $scope.interface.Nameservers.filter(
            Boolean
          );
          // toString() is a cheap way to compare 2 string arrays
          if (
            $scope.interface.Nameservers.toString() !=
          ...
          if (!$scope.interface.DHCPEnabled) {
            ...
            $scope.interface.ipv4.values.forEach((value, index) => {
              if (!APIUtils.validIPV4IP(value['Address'])) {
                toastService.error(value.Address + ' invalid IP parameter');
                $scope.loading = false;
                return;
              }
              if (!APIUtils.validIPV4IP(value['Gateway'])) {
                toastService.error(
                  value.Address + ' invalid gateway parameter'
                );
                $scope.loading = false;
                return;
              }
              ...
              if (!value['PrefixLength']) {
                toastService.error(
                  value.Address + ' invalid Prefix Length parameter'
                );
                $scope.loading = false;
                return;
              }
              if (
                ...
                if (value.id) {
                  promises.push(updateIPV4(index));
                } else {
                  promises.push(addIPV4(index));
                }
              }
            });
          }
        ...
        function setMACAddress() {
          return APIUtils.setMACAddress(
            $scope.selectedInterface,
            $scope.interface.MACAddress
          ).then(
            function(data) {},
            function(error) {
        ...
        function setDefaultGateway() {
          return APIUtils.setDefaultGateway($scope.defaultGateway).then(
            function(data) {},
            function(error) {
              console.log(JSON.stringify(error));
              return $q.reject();
        ...
        function setHostname() {
          return APIUtils.setHostname($scope.hostname).then(
            function(data) {},
            function(error) {
              console.log(JSON.stringify(error));
              return $q.reject();
        ...
        function setDHCPEnabled() {
          return APIUtils.setDHCPEnabled(
            $scope.selectedInterface,
            $scope.interface.DHCPEnabled
          ).then(
            function(data) {},
            function(error) {
              console.log(JSON.stringify(error));
        ...
        function setNameservers() {
          ...
          return APIUtils.setNameservers(
            $scope.selectedInterface,
        ...
        function removeIPV4s() {
          return $scope.ipv4sToDelete.map(function(ipv4) {
            return APIUtils.deleteIPV4($scope.selectedInterface, ipv4.id).then(
              function(data) {},
              function(error) {
                console.log(JSON.stringify(error));
                return $q.reject();
        ...
        function loadNetworkInfo() {
          APIUtils.getNetworkInfo().then(function(data) {
            ...
            if ($scope.network.interface_ids.length) {
              ...
              // page
              $scope.oldInterface = JSON.parse(
                JSON.stringify($scope.interface)
              );
            }
