  ...
  angular.module('app.configuration').controller('certificateController', [
    ...
    function ($scope, APIUtils, $q, Constants, toastService, $timeout) {
      ...
      $scope.getCSRCode = function () {
        ...
        //if user provided a first alternative name then push to alternative names array
        $scope.newCSR.firstAlternativeName ? alternativeNames.push($scope.newCSR.firstAlternativeName) : $scope.newCSR.firstAlternativeName = "";


        addCSR.CertificateCollection = { "@odata.id": $scope.newCSR.certificateCollection.location || "" };
        ...
        console.log('addCSR.Country: ', addCSR.Country);
        ...
        APIUtils.createCSRCertificate(addCSR).then(
          ...
          function (error) {
            $scope.addCSRModal = false;
            toastService.error('Unable to generate CSR. Log out and try again.');
            console.log(JSON.stringify(error));
          }
        )
      }

      //resetting the modal when user clicks cancel/closes the modal
