  ...
  angular.module('app.accessControl').controller('ldapController', [
    ...
    function($scope, APIUtils, $q, toastService) {
      ...
      $scope.saveLdapSettings = function() {
        ...
        const enabledServicePayload = getLdapRequest(enabledServiceType, true);
        const disabledServiceType =
            enabledServiceType == 'ldap' ? 'ad' : 'ldap';
        const disabledServicePayload =
            getLdapRequest(disabledServiceType, false);
      ...
      function getLdapRequest(serviceType, serviceEnabled) {
        ...
        if (serviceEnabled) {
          ...
          if (serviceType == 'ldap') {
            ldapProperties = {LDAP: {ServiceEnabled: serviceEnabled}};
          } else {
            ldapProperties = {
              ActiveDirectory: {ServiceEnabled: serviceEnabled}
            };
          }
        }
