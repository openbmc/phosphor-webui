  ...
  angular.module('app.accessControl').controller('ldapController', [
    ...
    function($scope, APIUtils, $q, toastService) {
      ...
      $scope.loadLdap = function() {
        $scope.loading = true;
        $scope.submitted = false;
        const ldapAccountProperties = APIUtils.getAllUserAccountProperties().then(
            function(data) {
        ...
        const promises = [ldapAccountProperties, ldapCertificate, caCertificate];
      ...
      function getUseSSL(ldapProperties) {
        ...
        if (serviceType) {
          isSSL =
              isSecureURI(ldapProperties[serviceType]['ServiceAddresses'][0]);
        }
        return isSSL;
      }
