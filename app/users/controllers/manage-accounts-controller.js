  ...
  angular.module('app.users').controller('manageAccountsController', [
    ...
    function($scope, $window, APIUtils, dataService) {
      ...
      $scope.createNewUser = function (){
          ...
          if(!$scope.selectedUser.UserName || !$scope.selectedUser.Password || !$scope.selectedUser.VerifyPassword)
          {
            $scope.state = 'error';
            $scope.outMsg = "Username or Password or verify password can't be empty";
            return;
          }
          if($scope.selectedUser.Password !== $scope.selectedUser.VerifyPassword)
          {
            $scope.state = 'error';
            $scope.outMsg = "Passwords not matching";
            return;
          }
          $scope.selectedUser.VerifyPassword = null;
          var data = {};
          ...
          APIUtils.createUser(data).then(
            function(response) {
              $scope.state = 'success';
              if(response.data["@Message.ExtendedInfo"] != undefined) {
                  $scope.outMsg = response.data["@Message.ExtendedInfo"][0].Message;
              }
              loadUserInfo();
            },
            function(error) {
              $scope.state = 'error';
              $scope.outMsg = error.error["@Message.ExtendedInfo"][0].Message;
              loadUserInfo();
            }
          );
      }
      $scope.updateUserInfo = function (){
          ...
          var data = {};
          if($scope.selectedUser.UserName !== $scope.selectedUser.CurrentUserName)
          {
            data["UserName"] = $scope.selectedUser.UserName;
          }
          $scope.selectedUser.VerifyPassword = null;
          ...
          APIUtils.updateUser($scope.selectedUser.CurrentUserName,data).then(
            function(response) {
              $scope.state = 'success';
              if(response.data["@Message.ExtendedInfo"] != undefined) {
                  $scope.outMsg = response.data["@Message.ExtendedInfo"][0].Message;
