...
window.angular && (function(angular) {
  ...
  angular.module('app.serverControl').controller('virtualMediaController', [
    ...
    function($scope, APIUtils, $q, toastService, dataService, nbdServerService) {
      var vms = [];
      ...
      APIUtils.getVMCollection().then(
        function(res) {
          ...
          for (var idx in vms) {
            promises.push(
            APIUtils.getVirtualMedia(vms[idx]["@odata.id"]).then(
              function(res) {
          ...
          $q.all(promises).finally(function() {
            ...
            console.log($scope.proxyDevices);
          });
        },function(error) {
          console.log(error);
          toastService.error(error);
      ...
      function createVMDevice(redfishData) {
        ...
        vmDevice.file = "";
        vmDevice.wsURI = dataService.getHost().replace('https://', 'wss://') + redfishData.Oem.WebSocketEndpoint;
      ...
      $scope.startVM = function(index) { 
        ...
        var server = new NBDServer($scope.proxyDevices[index].wsURI, file, id, index);
...
class NBDServer {
  constructor(endpoint, file, id, index) {
    ...
    this._on_ws_close = function (ev) {
      console.log(endpoint + ' closed with code: ' + ev.code +
        ' reason: ' + ev.reason);
      $scope.$apply(function () {
        if (ev.code == 1000) {
          $scope.proxyDevices[index].message = 'Server closed successfully';
        }
        else {
          $scope.proxyDevices[index].message = 'Server closed with errors';
