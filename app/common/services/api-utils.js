      ...
      var SERVICE = {
              ...
              .then(
                  function(response) {
                    ...
                    function parseNetworkData(content) {
                      ...
                      for (var key in content.data) {
                        if (key.match(/network\/eth\d+$/ig)) {
                          ...
                          if (data.interfaces[interfaceId][interfaceType].ids.indexOf(
                                  interfaceHash) == -1) {
                    ...
                    if (content.data.hasOwnProperty('/xyz/openbmc_project/network/config') &&
                        content.data['/xyz/openbmc_project/network/config'].hasOwnProperty(
                            'HostName')) {
                    ...
                    if (content.data.hasOwnProperty('/xyz/openbmc_project/network/eth0') &&
                        content.data['/xyz/openbmc_project/network/eth0'].hasOwnProperty(
                            'MACAddress')) {
        ...
        changePassword: function(user, newPassword) {
          ...
          $http({
            method: 'POST',
            url:
                DataService.getHost() + '/xyz/openbmc_project/user/' + user + '/action/SetPassword',
            headers: {'Accept': 'application/json', 'Content-Type': 'application/json'},
            withCredentials: true,
            ...
            responseType: 'arraybuffer'
              ...
              .then(
                  function(response, status, headers) {
                    deferred.resolve({data: response, status: status, headers: headers});
                  },
                  function(error) {
                    console.log(error);
                    deferred.reject(error);
                        ...
                        data.push(Object.assign(
                            {
                              ...
                              selected: false,
                              search_text: ('#' + content.data[key].Id + ' ' + severityCode + ' ' +
              ...
              .then(
                  function(response) {
                    ...
                    function getSensorStatus(reading) {
                      var severityFlags = {critical: false, warning: false, normal: false},
                    ...
                    for (var key in content.data) {
                      if (content.data.hasOwnProperty(key) &&
                        ...
                        content.data[key].Value =
                            getScaledValue(content.data[key].Value, content.data[key].Scale);
                        content.data[key].CriticalLow =
                            getScaledValue(content.data[key].CriticalLow, content.data[key].Scale);
                        content.data[key].CriticalHigh =
                            getScaledValue(content.data[key].CriticalHigh, content.data[key].Scale);
                        content.data[key].WarningLow =
                            getScaledValue(content.data[key].WarningLow, content.data[key].Scale);
                        content.data[key].WarningHigh =
                            getScaledValue(content.data[key].WarningHigh, content.data[key].Scale);
                        ...
                        sensorData.push(Object.assign(
                            {
                              ...
                              search_text:
                                  (title + ' ' + content.data[key].Value + ' ' +
                                   ...
                                   severity.severityText + ' ' + content.data[key].CriticalLow +
                                   ' ' + content.data[key].CriticalHigh + ' ' +
                                   content.data[key].WarningLow + ' ' +
                                   content.data[key].WarningHigh + ' ')
                                      .toLowerCase(),
        ...
        getActivation: function(imageId) {
          return $http({
                   method: 'GET',
                   url: DataService.getHost() + '/xyz/openbmc_project/software/' + imageId +
                       '/attr/Activation',
                   headers: {'Accept': 'application/json', 'Content-Type': 'application/json'},
                   withCredentials: true
                 })
                  ...
                  function(response) {
                    ...
                    function getFormatedExtendedVersions(extendedVersion) {
                      ...
                      extendedVersion.forEach(function(item) {
                        ...
                        titlePart =
                            titlePart[0].toUpperCase() + titlePart.substr(1, titlePart.length);
                    ...
                    if (content.data[Constants.FIRMWARE.FUNCTIONAL_OBJPATH]) {
                      functionalImages =
                          content.data[Constants.FIRMWARE.FUNCTIONAL_OBJPATH].endpoints;
                    }
                    for (var key in content.data) {
                      if (content.data.hasOwnProperty(key) &&
                        ...
                        isExtended = content.data[key].hasOwnProperty('ExtendedVersion') &&
                            content.data[key].ExtendedVersion != '';
                        if (isExtended) {
                          extendedVersions =
                              getFormatedExtendedVersions(content.data[key].ExtendedVersion);
                        }
                        data.push(Object.assign(
                            {
                              path: key,
          ...
          $http({
            ...
            function titlelize(title) {
              ...
              for (var i = 0; i < Constants.HARDWARE.uppercase_titles.length; i++) {
                if (title.toLowerCase().indexOf((Constants.HARDWARE.uppercase_titles[i] + ' ')) >
                    -1) {
        ...
        deleteLogs: function(logs) {
          ...
          logs.forEach(function(item) {
            promises.push($http({
              method: 'POST',
              url: DataService.getHost() + '/xyz/openbmc_project/logging/entry/' + item.Id +
                  '/action/Delete',
              headers: {'Accept': 'application/json', 'Content-Type': 'application/json'},
              withCredentials: true,
              data: JSON.stringify({'data': []})
          ...
          return $http({
              ...
              .then(
                  function(response) {
                    ...
                    return getScaledValue(content.data.Value, content.data.Scale) + ' ' +
