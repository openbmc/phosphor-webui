  ...
  (function(angular) {
    ...
    angular.module('app.common.services').factory('APIUtils', [
      ...
      function($http, Constants, $q, DataService, $interval) {
        const getScaledValue = function(value, scale) {
          scale = scale + '';
          scale = parseInt(scale, 10);
          const power = Math.abs(parseInt(scale, 10));
        ...
        var SERVICE = {
          ...
          getRedfishSysName: function() {
            return $http({
              ...
              function(response) {
                const sysUrl = response.data['Members'][0]['@odata.id'];
          ...
          pollHostStatusTillOn: function() {
            const deferred = $q.defer();
            const hostOnTimeout = setTimeout(function() {
              ws.close();
              deferred.reject(
            ...
            ws.onmessage = function(evt) {
              ...
              const hostState = content.properties.CurrentHostState;
          ...
          pollHostStatusTilReboot: function() {
            const deferred = $q.defer();
            const onState = Constants.HOST_STATE_TEXT.on_code;
            const offState = Constants.HOST_STATE_TEXT.on_code;
            let hostTimeout;
            const setHostTimeout = function(message, timeout) {
          ...
          pollHostStatusTillOff: function() {
            const deferred = $q.defer();
            const hostOffTimeout = setTimeout(function() {
              ws.close();
              deferred.reject(
          ...
          setSNMPManagerAddress: function(snmpManagerPath, address) {
            return $http({
              method: 'PUT',
              url: DataService.getHost() + snmpManagerPath + '/attr/Address',
              withCredentials: true,
                  ...
                  for (const key in content.data) {
                    if (key.match(/network\/eth\d+(_\d+)?$/gi)) {
                      ...
                      if (data.interface_ids.indexOf(interfaceId) == -1) {
                        ...
                        data.interfaces[interfaceId].DomainName = content.data[
                          key
                        ].DomainName.join(' ');
                      ...
                      key.match(
                        /network\/eth\d+(_\d+)?\/ipv[4|6]\/[a-z0-9]+$/gi
                      )
                    ) {
                      keyParts = key.split('/');
                      ...
                      if (
                        data.interfaces[interfaceId][interfaceType].ids.indexOf(
                          interfaceHash
                        ) == -1
                      ) {
                        data.interfaces[interfaceId][interfaceType].ids.push(
                          interfaceHash
                        );
                        data.interfaces[interfaceId][interfaceType].values.push(
                          content.data[key]
                        );
          ...
          getAccountServiceRoles: function() {
            ...
            return $http({
              method: 'GET',
              url: DataService.getHost() + '/redfish/v1/AccountService/Roles',
              withCredentials: true,
            }).then(
              function(response) {
          ...
          saveUserAccountProperties: function(
            lockoutduration,
            lockoutthreshold
          ) {
            const data = {};
          ...
          getLogs: function() {
            ...
            $http({
              ...
              function(response) {
                const json = JSON.stringify(response.data);
                const content = JSON.parse(json);
                const dataClone = JSON.parse(JSON.stringify(content.data));
                const data = [];
                let severityCode = '';
                let priority = '';
                const health = '';
              ...
              function(response) {
                ...
                for (const key in content.data) {
                  if (
                    ...
                    if (
                      Constants.SENSOR_SORT_ORDER.indexOf(
                ...
                for (const key in content.data) {
                  if (
                    ...
                    if (content.data[key].Activation) {
                      activationStatus = content.data[key].Activation.split(
                        '.'
                      ).pop();
                    }
                    ...
                    isExtended =
                      content.data[key].hasOwnProperty('ExtendedVersion') &&
                      ...
                      Object.assign(
                        {
                          ...
                          extended: {show: false, versions: extendedVersions},
                          data: {key: key, value: content.data[key]},
                        },
                        content.data[key]
                      )
          ...
          getServerInfo: function() {
            ...
            return $http({
              method: 'GET',
              url:
                DataService.getHost() + '/xyz/openbmc_project/inventory/system',
              withCredentials: true,
            }).then(function(response) {
              return response.data;
            });
          ...
          getTime: function() {
            return $http({
              method: 'GET',
              url:
                DataService.getHost() + '/xyz/openbmc_project/time/enumerate',
              withCredentials: true,
            }).then(function(response) {
              return response.data;
            });
          ...
          getHardwares: function(callback) {
            $http({
              ...
              function isSubComponent(key) {
                for (
                  let i = 0;
                  i < Constants.HARDWARE.parent_components.length;
                  i++
                ) {
                  if (
                    key.split(Constants.HARDWARE.parent_components[i]).length ==
                    2
                  )
                    return true;
                }
              ...
              function titlelize(title) {
                ...
                for (
                  let i = 0;
                  i < Constants.HARDWARE.uppercase_titles.length;
                  i++
                ) {
                  if (
                    title
                      .toLowerCase()
                      .indexOf(Constants.HARDWARE.uppercase_titles[i] + ' ') >
                    -1
                  ) {
              ...
              for (let i = 0; i < hardwareData.length; i++) {
                if (!keyIndexMap[hardwareData[i].parent]) {
                  ...
                  for (let j = 0; j < orderedComponents.length; j++) {
                    if (orderedComponents[j].path === hardwareData[i].parent) {
                      const child = hardwareData[i];
          ...
          getPowerConsumption: function() {
            return $http({
              ...
              function(response) {
                ...
                return (
                  getScaledValue(content.data.Value, content.data.Scale) +
                  ' ' +
