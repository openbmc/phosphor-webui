      ...
      return {
        ...
        'controller': [
          ...
          function($scope, APIUtils, toastService, Constants) {
            ...
            /**
             * This function is needed to map the backend Description to what should
             * appear in the GUI. This is needed specifically for CA certificate types.
             * The backend description for the certificate type is 'TrustStore Certificate',
             * this function will make sure we display 'CA Certificate' on the frontend
             * @param {string} : certificate Description property
             * @returns {string} : certificate name that should appear on frontend
             */
                ...
                APIUtils.replaceCertificate(data).then(
                    function(data) {
                      ...
                      toastService.success(
                          $scope.getCertificateName(certificate.Description) + ' was replaced.');
                      $scope.reload();
                    },
                    function(error) {
                      console.log(error);
                      $scope.loading = false;
                      toastService.error(
                          'Unable to replace ' +  $scope.getCertificateName(certificate.Description));
                    });
              };
