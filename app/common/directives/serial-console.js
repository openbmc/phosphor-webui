...
var termConfigJSON = require('../../../terminal.config.json');
...
function customKeyHandlers(ev) {
  for (var i = 0; i < termConfigJSON.keyNumber; i++) {
    if (ev.keyCode == termConfigJSON.keys[i].keyCode) {
      return callbackArray[termConfigJSON.keys[i].keyHandler](ev, this);
    }
  }
  return true;
}
      ...
      return {
        ...
        'controller': [
          ...
          function($scope, $window, dataService) {
            ...
            term.fit();
            if (termConfigJSON.customKeyEnable == true) {
